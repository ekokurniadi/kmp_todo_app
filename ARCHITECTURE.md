...
| Dependency Injection (Koin Annotations + KSP) | `feature/todo/di/TodoModule.kt`, `core/di/CoreModule.kt`, `di/Initializer.kt`, generated under `org.koin.ksp.generated` |
...
5. Wire it up (Koin Annotations):
   - Create a `class <Name>Module` in `feature/<name>/di/` annotated `@Module`.
   - Annotate each provider function: `@Single` / `@Factory` / `@KoinViewModel`. Use `@Named` for typed qualifiers; use `@InjectedParam` for runtime parameters (e.g. a route argument).
   - Add `<Name>Module().module` to the `modules(...)` list in `di/Initializer.kt` (the `.module` extension is generated by KSP under `org.koin.ksp.generated`).
